...
export class LinkService {
  ...
  private linksUrl = 'http://localhost:8080/links';
  ...
  postLink(linkAPI: LinkAPI): Observable<LinkDB> {
    return this.http.post<LinkDB>(this.linksUrl, linkAPI, this.httpOptions).pipe(
      tap((newLink: LinkDB) => this.log(`added link w/ id=${newLink.ID}`)),
      catchError(this.handleError<LinkDB>('addLink'))
    );
  }
  ...
  updateLink(linkdb: LinkDB): Observable<LinkDB> {
    ...
    return this.http.put(url, linkdb, this.httpOptions).pipe(
      tap(_ => this.log(`updated linkdb id=${linkdb.ID}`)),
